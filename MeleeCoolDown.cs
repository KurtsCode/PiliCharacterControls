/*
 * Author: Kurt Noe
 * Desc: Controller script for the melee attack. Check for when the Left Mouse Buttom is pressed to then activate the melee ability and pass down the proper values os that it can be executed in the direction the
 *       character is currently facing. Locks are set in place to ensure that the melee attack cannot be initiated from pressing the LMB during certain actions such as dashing or clicking in menus.
 *       Attached to the player character and calls the MeleeAbility script when the attack button is pressed.
 * Version: 1.0
 */
...
public class MeleeCoolDown : MonoBehaviour // Used to establish the cooldown time of the ability and trigger it once the button is pressed. Also designates audio and visuals when needed.
{

    public string abilityButtonAxisName = "Fire1";    // Appears in Inspector
   ...
   // private AudioSource abilitySource;
    ...
    [HideInInspector] public bool attackLock;
    ...
    void Start()
    {
        ...
        Initialize(ability, weaponHolder); // Takes the ability and the spawn point and assigns them to the value.
    }
